...
const comboContainer = 'slds-combobox_container';
const hasSelection = ' slds-has-selection';
const comboBox = 'slds-combobox slds-dropdown-trigger slds-dropdown-trigger_click';
const isOpen = ' slds-is-open';
const comboForm = 'slds-combobox__form-element slds-input-has-icon';
...
export default class Lookup extends LightningElement {
    ...
    @track comboContainerClass = comboContainer;
    @track comboBoxClass = comboBox;
    @track comboFormElmClass = comboForm+hasIconRight;
    @track entityIcon = {};
    ...
    connectedCallback() {
        ...
        this._comboContainer += ' '+this._iconSize;
        this.comboContainerClass = this._comboContainer;
    ...
    handleTriggerClick(event) {
        ...
        if(!this.isOptionSelected && !this._readOnly && !this.disabled){
            this.showOptions();
            this.inputElement.focus();
        }
    }
    ...
    showOptions(){
        ...
        this.comboBoxClass = comboBox+isOpen;
    }
    ...
    handleKeyChange(event) {
        ...
        if(searchKey){            
            ...
            if(this.comboBoxClass !== comboBox+isOpen) 
                this.comboBoxClass = comboBox+isOpen; //To show the options dropdown
    ...
    handleSelection(event){
        ...
        this.dispatchEvent(new CustomEvent('change', {detail: this.selectedOption, bubbles: true})); //Dispatch change event
    }
    ...
    showSelectedOption(obj){
        ...
        this.formElementClass = this._formElement;
        this.comboContainerClass += hasSelection; //Add styles to the selected option
        this.comboBoxClass = comboBox; //To close the Options dropdown
        this.comboFormElmClass = comboForm;
        this.comboFormElmClass += this.hasIcon ? hasIconBothSides : hasIconRight; //Handles the option icon and close icon
    ...
    removeSelectedOption(){
        ...
        this.comboContainerClass = this._comboContainer; //Reset the search box style
        this.comboFormElmClass = comboForm+hasIconRight; //To show the search icon only
        this.dispatchEvent(new CustomEvent('change', {detail: this.selectedOption, bubbles: true})); //Dispatch change event
    }
